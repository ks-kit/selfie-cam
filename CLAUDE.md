...
| 4 | 色味フィルター（7種・強さ調整つき） | **完了**（LUT 画像ではなく式で実装。理由は `仕様書.md`） |
...
**Phase 3（顔検出）が唯一残っている「できていないこと」。**
肌の判定を色だけで行っているため、**唇（マスク値 1.00）と木目などの暖色の背景（1.00）が
肌と区別できない**。この2つだけは色では分離できず、顔検出が要る。
今は個別のガード（唇は赤の超過で除外、など）で凌いでいる。

Phase 6（動画撮影）は要望としては弱い。
